...
let searchInput = document.querySelector('.find-recipe');
let searchForm = document.querySelector('.search');

searchButton.addEventListener('click', searchHandler);
searchForm.addEventListener('submit', searchHandler);

function filterRecipes(query, recipesToFilter) {
    const lowerCaseQuery = query.toLowerCase();

    const filteredRecipes = recipesToFilter.filter(function(recipe) {
        ...
        const tagsMatch = recipe.tags.some(tag => tag.toLowerCase().includes(lowerCaseQuery));
...
function searchHandler(e) {
    ...
    const recipeQuery = searchInput.value;
    const filteredAndSortedRecipes = filterRecipes(recipeQuery, recipes);
...
function renderRecipes(recipeList) {
    ...
    if (recipeList.length === 0) {
        allRecipesHtml = '<p>No recipes found matching your search.</p>';
    } else {
        for (let i = 0; i < recipeList.length; i++) {
            allRecipesHtml += recipeTemplate(recipeList[i]);
        }
    }

    if (recipeContainer) {
        recipeContainer.innerHTML = allRecipesHtml;
    } else {
        console.error("Error: Could not find element with class '.recipe-output-container'");
    }
}
